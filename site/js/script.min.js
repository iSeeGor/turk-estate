...
$(function () {
  ...
  ///
  intersectionObserverApi(".map-wrapper .map-holder", resizeMapContainer);
  intersectionObserverApi(".testimonialSlider__message-inner", testimonialMessageScrollBar);
...
var similarProperySlider = function similarProperySlider() {
  ...
  const enableSwiper = function() {
      mySwiper = new Swiper(".related-block .property-slider__container", {
          wrapperClass: "property-slider__body",
          slideClass: "property-slider__slide",
          speed: 600,
          slidesPerView: "auto",
...
var metaDropdown = function metaDropdown() {
  ...
  $(document).on('click', '.meta-dropdown__list-item', function () {
    var html = $(this).html();
    $(this).parent().parent().find('.meta-dropdown__list-item').removeClass('meta-dropdown__list-item_current');
...
var popupsRun = function popupsRun() {
  ...
  $(document).on('click', '.js-open-popup', function () {
    ...
    var popupTitle = popup.find('.popup-header__title');
    var popupImage = popup.find('.popup__image');
    var dataID = $this.data('popup-object-id');
    var dataTitle = $this.data('popup-title');
    var dataImage = $this.data('popup-image');
...
var readMoreText = function readMoreText() {
  ...
  var newText = '';
  var space = '';
  var textBlock = $('.js-readMore-text');
  var btn = "<a href=\"#\" class=\"moreLess-btn\">\u041F\u043E\u043A\u0430\u0437\u0430\u0442\u044C \u0432\u0441\u0435</a>";
  textBlock.each(function () {
    ...
    if (allText.length > charLim) {
      ...
      // space = allText.substr(700,allText.indexOf(' '));
      var count = allText.substring(charLim, allText.length);
      space = allText.substr(charLim,count.indexOf('. ')).length+1;
      firsHalf = allText.substring(0, charLim+space);
      lastHalf = allText.substring(charLim+space, allText.length);
      // var arr = allText.split(' ');
      // firsHalf = arr.slice(0, 200).join(' ');
      // lastHalf = arr.slice(200, arr.length).join(' ');
      newText = firsHalf+ '<div class="readMore-allTxt">' + lastHalf + "</div>";
      $this.html(newText);
      $this.append(btn);
...
const googleMap = () => {
    ...
    if(typeof address != 'undefined'){
        content = '<p class="marker_content">'+address+'</p>'
    }

  function initMap() {
      let popupContent = content,
          image = {
              url: '/site/images/icons/google-map-marker.svg'
